 ...
 * @since          2012-09-16
 * @version        1.4
 * @description    Call the main javascipt functions of the normal Visual Guide view
    ...
    if (title != null && catNo != null) {
        $.ajax({
           ...
           success: function(response) {
               ...
               {
                   if (!showMap) { // If it is possible to show more than one room, remove the if-Statement and the boolean variable showMap
        ...
        $.error("No parameters: Can't request target coordinates/room.");
    }
});
